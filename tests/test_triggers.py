...
def test_run_tier_is_min_of_author_and_spec():
    ...
    run = plan_triggered_run(
        c, _author(tier="frontier"), subject="prof", binding=c.trigger_bindings[0]
    )
...
def test_disjoint_scope_refuses_to_bind():
    ...
    with pytest.raises(TriggerError):
        plan_triggered_run(
            c, _author(scope="physics"), subject="prof", binding=c.trigger_bindings[0]
        )
...
def test_obo_user_is_the_author():
    ...
    run = plan_triggered_run(
        c, _author(tenant="chem"), subject="prof", binding=c.trigger_bindings[0]
    )

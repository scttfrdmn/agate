...
def test_undeclared_tool_action_is_denied(iam_client):
    ...
    c = compile_agent(
        _hpc_spec(["hpc-monitor"]), region=REGION, account=ACCOUNT, gateway_arn=GATEWAY_ARN
    )
    d = _eval(
        iam_client,
        c.tool_policy,
        action="bedrock-agentcore:CreateGateway",  # not granted
        resource="arn:aws:bedrock-agentcore:us-east-1:111122223333:gateway/agate-hpc",
        tags=_tags(),
    )
...
def test_skill_granted_gateway_tool_is_invocable(iam_client):
    ...
    c = compile_agent(
        _skill_spec(["hpc-analyst"]), region=REGION, account=ACCOUNT, gateway_arn=GATEWAY_ARN
    )
    d = _eval(
        iam_client,
        c.tool_policy,
